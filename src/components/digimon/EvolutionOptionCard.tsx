...
export function EvolutionOptionCard({
  ...
  const attributeIcon = useMemo(() => resolveAttributeIcon(attributeLabel), [attributeLabel]);
          ...
          <span className="flex items-center gap-2 text-xs text-muted-foreground">
            {attributeLabel ? (
              <span className="inline-flex items-center gap-1">
                {attributeIcon ? <img src={attributeIcon} alt="" className="h-4 w-4" loading="lazy" /> : null}
...
export function EvolutionRequirementsPreview({ rows }: EvolutionRequirementsPreviewProps) {
  if (!rows.length) {
