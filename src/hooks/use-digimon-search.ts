...
export function useDigimonSearch(list: DigimonSummary[], query: string, options: UseDigimonSearchOptions = {}) {
  ...
  return useMemo(() => {
    ...
    return list
      .filter((entry) => {
        ...
        const attributeMatch = includeAttribute ? entry.attribute?.toLowerCase().includes(trimmed) : false;
        const generationMatch = entry.generation?.toLowerCase().includes(trimmed);
