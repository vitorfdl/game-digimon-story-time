...
function resolveRequestUrl(path: string): string {
  ...
  const proxyBase = (typeof import.meta !== "undefined" && import.meta.env?.VITE_GRINDOSAUR_PROXY)
...
export async function fetchDigimonList(): Promise<DigimonSummary[]> {
  ...
  const response = await fetch(resolveRequestUrl(DIGIMON_LIST_PATH), fetchOptions)
  if (!response.ok) {
 ...
 */
export async function fetchDigimonDetails(slug: string): Promise<DigimonDetails> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to load Digimon page for ${slug} (${response.status})`)
  }
...
function parseQuickFacts($: CheerioAPI) {
  const table = $("table")
    .filter((_, element) => $(element).find("caption").text().trim() === "Quick Facts")
    .first()
...
function parseAttachmentSkills($: CheerioAPI): AttachmentSkillRow[] {
  ...
  if (!table.length) {
    ...
    const hasNoSkillsMessage = section
      .find("p")
      .filter((_, p) => $(p).text().toLowerCase().includes("does not possess"))
      .length
...
function findHeading($: CheerioAPI, label: string) {
  ...
  return $("h2, h3").filter((_, element) => $(element).text().trim().toLowerCase() === lowerLabel)
}
