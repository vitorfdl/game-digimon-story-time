      ...
      {slots.length ? (
        <>
          <TeamBuilderBoard
            ...
            onReorder={(activeId, overId) => {
              setSlots((current) => {
                const oldIndex = current.findIndex((slot) => slot.id === activeId);
        ...
        <div className="grid place-items-center rounded-3xl border border-dashed border-border/50 bg-muted/20 px-6 py-12 text-center text-sm text-muted-foreground">
          <div className="space-y-4">
            ...
            <p>Add your first partner to begin crafting evolution routes and synergistic personalities.</p>
            <AddDigimonButton list={list} onSelect={handleAdd} />
    ...
    <div className="mx-auto flex w-full max-w-7xl flex-col gap-6 px-2 pb-10">
      <div className="grid gap-4">
        {Array.from({ length: 3 }).map((_, index) => (
          <Skeleton key={index} className="h-64 rounded-3xl border border-border/50" />
        ))}
      </div>
    </div>
  );
